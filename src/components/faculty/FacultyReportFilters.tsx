import { Search, X, RotateCcw, Filter, Star, FolderKanban, GraduationCap, CheckCircle2 } from 'lucide-react'
import { JHARKHAND_DISTRICTS } from '../../data/jharkhandData'

export type FacultyViewMode = 'all' | 'interested' | 'projects' | 'assigned' | 'completed'

interface FacultyReportFiltersProps {
  searchQuery: string
  ...
  viewMode: FacultyViewMode
  onViewModeChange: (mode: FacultyViewMode) => void
...
export function FacultyReportFilters({
  searchQuery,
  onSearchChange,
  selectedDistrict,
  onDistrictChange,
  ...
  return (
    <div className="space-y-4 rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
      ...
      <div className="flex flex-wrap items-center gap-2 border-b border-slate-200 pb-3">
        ...
        >
          <Star size={13} className={viewMode === 'interested' ? 'fill-white' : 'fill-amber-500 text-amber-500'} />
        ...
        >
          ...
          <span>Faculty Project List</span>
          <span
            className={`rounded-full px-2 py-0.5 text-[11px] ${
              viewMode === 'projects' ? 'bg-white/20 text-white' : 'bg-teal-100 text-teal-900'
            }`}
          >
            {projectsCount}
          </span>
        ...
        >
          ...
          <span
            className={`rounded-full px-2 py-0.5 text-[11px] ${
              viewMode === 'assigned' ? 'bg-white/20 text-white' : 'bg-indigo-100 text-indigo-900'
            }`}
          >
            {assignedCount}
          </span>
      ...
      <div className="grid grid-cols-1 gap-2.5 sm:grid-cols-2 lg:grid-cols-4">
        {/* District Filter */}
        <div>
          <label htmlFor="faculty-filter-district" className="mb-1 block text-[11px] font-bold uppercase tracking-wider text-slate-500">
            District
          </label>
          <select
            id="faculty-filter-district"
        ...
        {/* Category Filter */}
        <div>
          <label htmlFor="faculty-filter-category" className="mb-1 block text-[11px] font-bold uppercase tracking-wider text-slate-500">
            Category
          </label>
          <select
            id="faculty-filter-category"
        ...
        {/* Urgency Filter */}
        <div>
          <label htmlFor="faculty-filter-urgency" className="mb-1 block text-[11px] font-bold uppercase tracking-wider text-slate-500">
            Urgency
          </label>
          <select
            id="faculty-filter-urgency"
        ...
        {/* Status Filter */}
        <div>
          <label htmlFor="faculty-filter-status" className="mb-1 block text-[11px] font-bold uppercase tracking-wider text-slate-500">
            Status
          </label>
          <select
